...
class test_mac_simple_duplex extends uvm_test;
  ...
  function void set_wishbone_config_params();
    ...
    if(!uvm_config_db #(virtual wb_m_bus_driver_bfm)::get(this, "", "WB_DRV_BFM", wb_config_0.wb_drv_bfm)) begin
       `uvm_fatal("BAD_CONFIG", "Cannot get() interface WB_DRV_BFM from uvm_config_db. Have you set() it?")
    end
    if(!uvm_config_db #(virtual wb_bus_monitor_bfm)::get(this, "", "WB_MON_BFM", wb_config_0.wb_mon_bfm)) begin
       `uvm_fatal("BAD_CONFIG", "Cannot get() interface WB_MON_BFM from uvm_config_db. Have you set() it?")
    end

    wb_config_0.m_wb_id = 0;  // WISHBONE 0
    wb_config_0.m_mac_id = MAC_MASTER_WB_ID;   // the WISHBONE bus master id of the MAC master
    wb_config_0.m_mac_eth_addr = 48'h000BC0D0EF00;
    wb_config_0.m_mac_wb_base_addr = MAC_SLAVE_WB_ID * SLAVE_ADDR_SPACE_SIZE; // base address of MAC slave
    wb_config_0.m_wb_master_id = 1; // the ID of the wb master
    wb_config_0.m_tb_eth_addr = 48'h000203040506;
    wb_config_0.m_s_mem_wb_base_addr = MEM_SLAVE_WB_ID * SLAVE_ADDR_SPACE_SIZE; // base address of slave mem
  ...
  function void set_mii_config_params();
    ...
    if(!uvm_config_db #(virtual mii_rx_driver_bfm)::get(this, "", "MIIM_IF_RX_DRV_BFM", mii_config_0.mii_rx_drv_bfm)) begin
      `uvm_fatal("BAD_CONFIG", "Cannot get() interface MII_RX_DRV_BFM from uvm_config_db. Have you set() it?")
    end

    if(!uvm_config_db #(virtual mii_tx_driver_bfm)::get(this, "", "MIIM_IF_TX_DRV_BFM", mii_config_0.mii_tx_drv_bfm)) begin
      `uvm_fatal("BAD_CONFIG", "Cannot get() interface MII_TX_DRV_BFM from uvm_config_db. Have you set() it?")
    end
