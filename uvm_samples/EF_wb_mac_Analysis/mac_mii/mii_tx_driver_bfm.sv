...
//----------------------------------------------
interface mii_tx_driver_bfm (mii_if m_v_miim_if);
...
`include "uvm_macros.svh"
  ...
  task get_frame(ethernet_txn txn);
    bit[31:0] crc32;
    logic [31:0] crc_received;  //crc from MAC
    logic [7:0]  temp_byte; // for assembling nibbles to bytes
    crc32 = 32'hffffffff;  //init CRC
    crc_received =0;  //crc from MAC
    `uvm_info("MII_TX_DRIVER_BFM","get_frame waiting for Tx to start",UVM_LOW )
    wait(m_v_miim_if.MTxEn);    // wait for transmission to start
    m_v_miim_if.MCrs = 1;    // set carrier sense

    `uvm_info("MII_TX_DRIVER_BFM", "Tx Preamble",UVM_LOW )
    for(int i=0; i<14; i++) begin  // preamble
      @(posedge m_v_miim_if.mtx_clk)
        if(m_v_miim_if.MTxD != 5) begin
          `uvm_error("MII_TX_DRIVER_BFM", "MTxEn Error in Preamble" )
          return;
        end
    end

    `uvm_info("MII_TX_DRIVER_BFM", "Tx SFD",UVM_LOW )
    @(posedge m_v_miim_if.mtx_clk)  // SFD
      if(m_v_miim_if.MTxD != 5 && m_v_miim_if.MTxEn) begin
        `uvm_error("MII_TX_DRIVER_BFM", "MTxEn Error in SFD" )
        return;
      end
    ...
    `uvm_info("MII_TX_DRIVER_BFM", "Tx Dest MAC addr",UVM_LOW )
    for(int i=47; i>0; i-=8)  begin         // Dest_addr (Dest MAC addr)
      @(posedge m_v_miim_if.mtx_clk)
        if(!m_v_miim_if.MTxEn) begin
          `uvm_error("MII_TX_DRIVER_BFM", "MTxEn Error in Dest Addr" )
          return;
        end
    ...
    `uvm_info("MII_TX_DRIVER_BFM", "Tx Source MAC addr",UVM_LOW )
    for(int i=47; i>0; i-=8)  begin         // srce_addr (Source MAC addr)
      @(posedge m_v_miim_if.mtx_clk)
        if(!m_v_miim_if.MTxEn) begin
          `uvm_error("MII_TX_DRIVER_BFM", "MTxEn Error in Source Addr" )
          return;
        end
    ...
    `uvm_info("MII_TX_DRIVER_BFM", "Tx length",UVM_LOW )
    for(int i=15; i>0; i-=8)  begin         // length
      @(posedge m_v_miim_if.mtx_clk)
        if(!m_v_miim_if.MTxEn) begin
          `uvm_error("MII_TX_DRIVER_BFM", "MTxEn Error in length" )
          return;
        end
    ...
    `uvm_info("MII_TX_DRIVER_BFM", "Tx Payload",UVM_LOW )
    txn.payload = new[txn.payload_size];  // resize payload
    for(int i=0; i<txn.payload_size; i++)  begin  // Payload
      @(posedge m_v_miim_if.mtx_clk)
        if(!m_v_miim_if.MTxEn) begin
          `uvm_error("MII_TX_DRIVER_BFM", "MTxEn Error in payload" )
          return;
        end
    ...
    `uvm_info("MII_TX_DRIVER_BFM", "Tx CRC",UVM_LOW )
    for(int i=31; i>0; i-=8)  begin        // CRC
      @(posedge m_v_miim_if.mtx_clk)
        if(!m_v_miim_if.MTxEn) begin
          `uvm_error("MII_TX_DRIVER_BFM", "MTxEn Error in CRC" )
          return;
        end
    ...
    @(posedge m_v_miim_if.mtx_clk)
      if(m_v_miim_if.MTxEn) begin
        `uvm_error("MII_TX_DRIVER_BFM", "MTxEn error" )
        return;
      end
      else
        m_v_miim_if.MCrs = 1;    // clear carrier sense

    //check crc against the "magic number" for the crc-32 polynomial
    txn.crc = crc_received;
    if(crc32 != 32'hc704dd7b)  //crc == magic number?
      `uvm_error("MII_TX_DRIVER_BFM","CRC error on received packet from MAC" )
  endtask
