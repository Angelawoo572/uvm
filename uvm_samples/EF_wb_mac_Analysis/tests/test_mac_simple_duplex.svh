...
class test_mac_simple_duplex extends uvm_test;
  ...
  function void set_wishbone_config_params();
    ...
    if(!uvm_config_db #(virtual wb_m_bus_driver_bfm)::get(this, "", "WB_drv_bfm", wb_config_0.WB_drv_bfm)) begin
      `uvm_fatal("SET_WISHBONE_CONFIG_PARAMS", "Unable to get WB_drv_bfm from configuration space")
    end
    if(!uvm_config_db #(virtual wb_bus_monitor_bfm)::get(this, "", "WB_mon_bfm", wb_config_0.WB_mon_bfm)) begin
      `uvm_fatal("SET_WISHBONE_CONFIG_PARAMS", "Unable to get WB_mon_bfm from configuration space")
    end
    wb_config_0.m_wb_id = 0;  // WISHBONE 0
    wb_config_0.m_mac_id = 0;   // the WISHBONE bus master id of the MAC master
    wb_config_0.m_mac_eth_addr = 48'h000BC0D0EF00;
    wb_config_0.m_mac_wb_base_addr = 32'h00100000; // base address of MAC slave
    wb_config_0.m_wb_master_id = 1; // the ID of the wb master
    wb_config_0.m_tb_eth_addr = 48'h000203040506;
    wb_config_0.m_s_mem_wb_base_addr = 32'h00000000; // base address of slave mem
    wb_config_0.m_mem_slave_size = 32'h00100000;  // default is 1 Mbyte
    wb_config_0.m_mem_slave_wb_id = 0;  // WISHBONE bus slave id of slave mem
  ...
  function void set_mii_config_params();
    ...
    if(!uvm_config_db #(virtual mii_rx_driver_bfm)::get(this, "", "MII_rx_drv_bfm", mii_config_0.MII_rx_drv_bfm)) begin
      `uvm_fatal("SET_MII_CONFIG_PARAMS", "Unable to get MII_rx_drv_bfm from configuration space")
    end

    if(!uvm_config_db #(virtual mii_tx_driver_bfm)::get(this, "", "MII_tx_drv_bfm", mii_config_0.MII_tx_drv_bfm)) begin
      `uvm_fatal("SET_MII_CONFIG_PARAMS", "Unable to get MII_tx_drv_bfm from configuration space")
    end
  ...
  task run_phase(uvm_phase phase);
    ...
    wb_config_0.wait_for_reset(); //wait for reset to clear on WISHBONE bus
    m_seq.start(wb_seqr_handle, null);  //start the main sequence
    wb_config_0.wait_for_clock(100);  //wait until they all are done
