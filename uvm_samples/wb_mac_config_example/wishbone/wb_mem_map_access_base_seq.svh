...
virtual class wb_mem_map_access_base_seq extends uvm_sequence;
  ...
  wb_config  m_cfg;  //Handle to wishbone config object
  ...
  // get_register_map
  //--------------------------------------------------------------------
  function void get_register_map();
  ...
  // get_wb_config
  //--------------------------------------------------------------------
  function void get_wb_config();
    if( m_cfg == null ) begin // get it from the sequencer
      if(!uvm_config_db #(wb_config)::get(m_sequencer, "", "wb_config", m_cfg)) begin
        `uvm_fatal("get_wb_config", "no config object for wb_config")
      end
    end
  endfunction
  ...
  function address_t get_address( string name );
    ...
    address = m_register_map.lookup_register_address_by_name( {m_register_map.get_name(), ".mac_0_regs.", name}, valid );
    if( !valid )
