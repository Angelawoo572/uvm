...
module stimuli_fsm (
    ...
    logic [DATA_W-1:0] registered_upper_bound;
    logic [DATA_W-1:0] registered_lower_bound;
    ...
    bounded_LFSR #(.W(DATA_W)) lfsr (
        ...
        .lo         (registered_lower_bound),
        .hi         (registered_upper_bound),
        .addr       (lfsr_output),
        .valid      (lfsr_valid),
        .diff       ()
    );
    ...
    always_comb begin
        lfsr_enable = 0;
        lfsr_seed_load = 0; // on seed_load, seed is loaded
        lfsr_req_load = 0; // on req_load, constraint_id and bounds are loaded
        stim_if.req_ready = 0;
        stim_if.rsp_valid = 0;
