    ...
    def generate_sv(self):
        ...
        if self.sub_modules:
            ...
            for sub_mod, instance_name in self.sub_modules:
                lines.append(f"    {sub_mod.name} {instance_name} (")
...
def generate_coverpoint(cp_data, counter_width=4):
    ...
    mod.add_line(f"logic [{counter_width-1}:0] ctr_r [{num_bins-1}:0];")
    mod.add_line(f"logic [{counter_width-1}:0] ctr_n [{num_bins-1}:0];")
    mod.add_line("")

    # Generate Outputs and map internal array to output ports
    for idx, b in enumerate(bins):
        bin_ref = b['reference']
        out_name = f"{bin_ref}_cnt"
    ...
    for i in range(num_bins):
        mod.add_line(f"        ctr_r[{i}] <= '0;")
    mod.add_line("    end else if (sample) begin")
    mod.add_line("        ctr_r <= ctr_n;")
    mod.add_line("    end")
    mod.add_line("end")
...
def generate_coverage_model(model_data):
    ...
    for cg_data in model_data['covergroups']:
        ...
        instance_name = f"{cg_data['reference']}_inst"
        mod.sub_modules.append((cg_mod, instance_name))
